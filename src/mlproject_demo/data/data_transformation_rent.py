...
class DataTransformationRent:
    def __init__(self):
        ...
        self.config = DataTransformationConfig(
            rent_data_path = cfg['transformed_rent']['rent_path'],
            rent_transformed_path = cfg['transformed_rent']['rent_transformed_path']
        )
        os.makedirs(self.config.rent_transformed_path,exist_ok = True)
        logger.info(f"Data Transformation for Rent is Initialized saved data in {self.config.rent_transformed_path}")
    

    def load_data(self):
        try:
            df = pd.read_csv(self.config.rent_data_path)
            logger.info(f"Rent Data loaded from path {self.config.rent_data_path} with shape {df.shape}")
    ...
    def run(self):
        try:
            ...
            np.save(os.path.join(self.config.rent_transformed_path,'X_train.npy'),X_train)
            np.save(os.path.join(self.config.rent_transformed_path,'X_test.npy'),X_test)
            np.save(os.path.join(self.config.rent_transformed_path,'y_train.npy'),y_train)
            np.save(os.path.join(self.config.rent_transformed_path,'y_test.npy'),X_test)

            save_object(preprocessor,os.path.join(self.config.rent_transformed_path,'preprocessor.pkl'))
...
if __name__ == "__main__":
    data_transformation = DataTransformationRent()
